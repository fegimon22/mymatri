...
class AppController extends Controller
{
	...
	$this->loadModel('State');
        ...
        if($this->userValue){
        $userValueArr=$this->Member->findById($this->userValue['Member']['id']);
        $profileCount=0;
        $totalProfile=0;
        $fieldArr=array('maritialstatus_id','name','sex','phone','address','dob','country_id','state_id','city_id','phone','height_id','religion_id','caste_id','employed_id','occupation_id','education_id','habit_id','mothertongue_id','rashy_id','about_me','profile_created','user_name','profileId','income','manglik','physical','children','complexion','body_type','birth_time','birth_place','father_occupation','mother_occupation','no_of_brother','no_of_sister','family_value','family_type');
        foreach($fieldArr as $value){
            $totalProfile++;
            if($userValueArr['Member'][$value]!=NULL){
                $profileCount++;
            }
        }
        $profilePercent=ceil(($profileCount*100)/$totalProfile);
        $this->set('profilePercent',$profilePercent);
