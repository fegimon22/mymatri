...
$serialNo = 1*$limit*($page-1)+1;?>
   ...
   <br/>
	    ...
	    if($this->Session->check('Member')){
	     echo$this->Html->link($this->Html->image($memberImg,array('class'=>'img-responsive img-thumbnail imageblur','alt' => $value['Viewprofile']['name'])),'javascript:void(0);',array('onclick'=>"show_modal('$url/view/$id/$age');",'escape' => false));
	     }else{
	     echo$this->Html->link($this->Html->image($memberImg,array('class'=>'img-responsive img-thumbnail imageblur','alt' => $value['Viewprofile']['name'])),array('controller'=>'Users','action'=>'login','Viewprofiles'),array('escape' => false,'title'=>$value['Viewprofile']['name'],'class'=>'religion_1'));
	     }?>
	...
	?>
	     ...
	     <?php $url=$this->Html->url(array('controller'=>'Viewprofiles'));
	    if($this->Session->check('Member')){
	     echo$this->Html->link($this->Html->image($memberImg,array('class'=>'img-responsive img-thumbnail imageblur','alt' => $value['Member']['name'])),'javascript:void(0);',array('onclick'=>"show_modal('$url/view/$id/$age');",'escape' => false));
	     }else{
	     echo$this->Html->link($this->Html->image($memberImg,array('class'=>'img-responsive img-thumbnail imageblur','alt' => $value['Member']['name'])),array('controller'=>'Users','action'=>'login','Viewprofiles'),array('escape' => false,'title'=>$value['Member']['name'],'class'=>'religion_1'));
	     }?>
             <div><strong class="text-danger"><?php echo __('Profile ID');?> : <?php echo $value['Member']['profileId'];?></strong></div>
		 <div><strong class="text-info"><?php 
		 if($viewed==1 || $value['Member']['id']==$memberId){
	     ...
	     <div class="">
			 ...
			 <?php 
			 if($viewed==1 || $value['Member']['id']==$memberId){ //echo $viewed.'-'.$value['Member']['id'];
				if($value['City']['name']){echo$value['City']['name'];}else{echo __('Not Specified');}?>,&nbsp;<?php if($value['State']['name']){echo$value['State']['name'];}else{echo __('Not Specified');}?>,&nbsp;<?php if($value['Country']['name']){echo$value['Country']['name'];}else{echo __('Not Specified');}
		}
		else{
			echo __('xxxxxxxxxx');
			//echo'&nbsp;&nbsp;'.$this->Form->button(__('Show Contact'),array('type'=>'button','id'=>'showcontact','class'=>'btn btn-sm'));
