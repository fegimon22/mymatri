...
class ProfilesController extends AppController
{
    ...
    public function editProfile()
    {
        ...
        if ($this->request->is(array('post', 'put')))
        {
			...
			$horoscopeName = $this->request->data['Profile']['horoscopeimg']['name'];
			$horoscopeTmp = $this->request->data['Profile']['horoscopeimg']['tmp_name'];
			...
			if(strlen($horoscopefileName)>0)
			{
				...
				$horoscoperecord=array('Profile'=>array('id'=>$this->Profile->id,'horoscope'=>$horoscopefileName));
				$this->Profile->save($horoscoperecord);
			}
			
			$proofName = $this->request->data['Profile']['proofimg']['name'];
			$proofTmp = $this->request->data['Profile']['proofimg']['tmp_name'];
			$prooffileName=$this->CustomFunction->upload($proofTmp,$proofName,'horoscope',$this->siteOrganization);
			if(strlen($prooffileName)>0)
			{
			 	//$this->request->data['Profile']['horoscopeimg']=$horoscopefileName;
				$proofrecord=array('Profile'=>array('id'=>$this->Profile->id,'proof'=>$prooffileName));
				$this->Profile->save($proofrecord);
			}
			
	
			
			//print_r($horoscopefileName);print_r($this->request->data['Profile']['horoscopeimg']['name']);die;
			
			$recordArr=array('Profile'=>array('id'=>$this->Profile->id,'photo_status'=>'Pending'));
			$this->Profile->save($recordArr);

		
                $this->Session->setFlash(__('Profile Updated Successfully and pending for admin approval.'),'flash',array('alert'=>'success'));
                $this->redirect(array('controller'=>'Dashboards','action' => 'index'));
            }
