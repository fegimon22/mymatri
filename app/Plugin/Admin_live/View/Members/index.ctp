...
<div class="panel-body">
			    ...
			    <th><?php echo __('Current Plan Detail')?></th>
			    ...
			    <?php }?>
                        ...
                        $id=$post['Member']['id'];
			$planArr=$this->Function->paymentStatus($id);
			if($post['Member']['photo'])
			    ...
			    <?php if($luserId==1){?>
                            ...
                            <td><?php
			    if($planArr){
			    echo '<strong>'.__('Plan Name').': </strong>'.h($planArr['Plan']['name']).'<br>
			    <strong>'.__('Expiry Date').': </strong>'.$this->Time->format($dtFormat,$planArr['MembersContact']['expiry_date']);}else{?>
			     <span class="label label-danger"><?php echo __('No Plan Purchase');?></span>
			   <?php }?>
			    </td>
			    <td><span class="label label-<?php if($planArr)echo"success";else echo"danger";?>"><?php if($planArr){echo __('Paid');}else{echo __('Unpaid');} ?></span></td>
                            <?php }?>
