<div <?php if(!$isError){?>class="container"<?php }?>>    
    <div class="panel panel-custom mrg">
        <div class="panel-heading"><?php echo __('Edit Weight');?><?php if(!$isError){?><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><?php }?></div>
        <div class="panel-body"><?php echo $this->Session->flash();?>
	    <?php echo $this->Form->create('Weight',array('class'=>'form-horizontal'));?>
	    <?php foreach ($Weight as $k=>$post): $id=$post['Weight']['id'];$form_no=$k+1;?>
		...
		<div class="panel-body">
			...
			<div class="col-sm-9">
			   <?php echo $this->Form->input("$k.Weight.name",array('label' => false,'class'=>'form-control','placeholder'=> __('Name'),'div'=>false));?>
			</div>
		    </div>
		    <div class="form-group text-left">
			<div class="col-sm-offset-3 col-sm-6">
			    <?php echo $this->Form->input("$k.Weight.id", array('type' => 'hidden'));?>
			</div>
		    </div>
		</div>
	    </div>
